...
function PrivacyContent({ data }: { data: WpPrivacyPage }) {
          ...
          >
            {heroData?.privacy_hero_image?.url && (
              <Image
                ...
                alt={heroData.privacy_hero_image.alt}
                fill
...
export default function PrivacyPolicy() {
  const [data, setData] = useState<WpPrivacyPage | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch using the slug or ID. The user provided the JSON for ID 379.
        const res = await fetch(
          "http://quintsnweb.local/wp-json/wp/v2/pages/379?_acf_format=standard"
        );
        const json = await res.json();

        // The endpoint for a specific ID returns an object directly
        if (json && json.acf) {
          setData(json);
        }
      } catch (error) {
  ...
  return <PrivacyContent data={data} />;
}
